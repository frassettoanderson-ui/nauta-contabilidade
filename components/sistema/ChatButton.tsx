    ...
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <div className={`w-full h-full rounded-full overflow-hidden flex items-center justify-center ${glow ? 'chat-unread-glow' : ''}`} style={{ background: 'var(--sys-surface-3)', border: '1px solid var(--sys-border-2)' }}>
        {foto
          ...
          : <span className="text-sm font-bold text-[color:var(--sys-accent)]">{(nome || '?').trim().charAt(0).toUpperCase()}</span>}
      </div>
    ...
    <>
        ...
        <div className="fixed bottom-5 left-5 z-50 flex flex-col gap-2.5 w-80">
          {toasts.map(t => (
            <button key={t.id} onClick={() => abrirToast(t)}
              ...
              style={{ background: 'var(--sys-modal)', border: `1px solid ${t.tipo === 'site' ? 'rgba(124,111,255,0.5)' : 'color-mix(in srgb, var(--sys-accent) 50%, transparent)'}` }}>
              <div className="w-11 h-11 rounded-full flex items-center justify-center shrink-0" style={{ background: t.tipo === 'site' ? 'rgba(124,111,255,0.15)' : 'color-mix(in srgb, var(--sys-accent) 15%, transparent)' }}>
                <MessageCircle size={20} style={{ color: t.tipo === 'site' ? '#a99bff' : 'var(--sys-accent)' }} />
              </div>
      ...
      <div className="fixed bottom-5 right-5 z-40">
        <button onClick={() => { pedirPermissaoNotif(); setOpen(o => !o) }} aria-label="Chat"
          ...
          style={{ background: 'linear-gradient(135deg, var(--sys-accent), #6355e0)', boxShadow: '0 4px 16px color-mix(in srgb, var(--sys-accent) 35%, transparent)' }}>
          ...
          <div className={`w-full lg:w-64 flex flex-col border-r ${ativo ? 'hidden lg:flex' : 'flex'}`} style={{ borderColor: 'var(--sys-border)' }}>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <button onClick={() => setSecEquipe(v => !v)} className="w-full flex items-center gap-2 px-3 pt-2.5 pb-1.5 text-left">
                ...
                {equipeUnread > 0 && <span className="ml-auto min-w-4 h-4 px-1 rounded-full bg-[color:var(--sys-accent)] text-white text-[9px] font-bold flex items-center justify-center">{equipeUnread}</span>}
              </button>
                ...
                {contatos.map(c => {
                  ...
                  return (
                    <button key={c.id} onClick={() => abrirContato(c)}
                      ...
                      {nl > 0 && <span className="min-w-5 h-5 px-1 rounded-full bg-[color:var(--sys-accent)] text-white text-[10px] font-bold flex items-center justify-center">{nl}</span>}
                    </button>
                  )
                })}
              </div></div>
              ...
              <>
                <div className="h-14 px-3 flex items-center gap-3 shrink-0 border-b" style={{ borderColor: 'var(--sys-border)' }}>
                  ...
                  <div className="min-w-0">
                    ...
                    {digitando ? <p className="text-[11px] text-[color:var(--sys-accent)] italic">digitando…</p> : <p className="text-[11px] text-gray-500">{ativo.subtitulo}</p>}
                  </div>
                  ...
                  {msgs.map(m => {
                    ...
                    return (
                      <div key={m.id} className={`flex animate-msg-in ${meu ? 'justify-end' : 'justify-start'}`}>
                        <div className="max-w-[78%] rounded-2xl px-3 py-2" style={{ background: meu ? 'linear-gradient(135deg, var(--sys-accent), var(--sys-accent-2))' : 'var(--sys-surface-3)' }}>
                          {m.arquivo_url && (ehImagem(m.arquivo_nome || m.arquivo_url)
                            ...
                            : <a href={m.arquivo_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm underline mb-1" style={{ color: meu ? '#fff' : 'var(--sys-accent)' }}><Paperclip size={13} /> {m.arquivo_nome || 'arquivo'}</a>)}
                ...
                <div className="p-2.5 flex items-center gap-2 shrink-0 border-t" style={{ borderColor: 'var(--sys-border)' }}>
                  ...
                  <button onClick={enviar} disabled={!texto.trim()} className="w-9 h-9 rounded-lg flex items-center justify-center text-white disabled:opacity-40" style={{ background: 'linear-gradient(135deg, var(--sys-accent), var(--sys-accent-2))' }}>
                    <Send size={16} />
                  </button>
                </div>
              </>

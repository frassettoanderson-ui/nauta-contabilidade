...
export default function Auditoria() {
  ...
  // scroll infinito: carrega mais registros ao rolar
  const lista = useScrollInfinito<LogAuditoriaItem>((p) => api.get<Pagina>(`/auditoria?${montarQs(p)}`), [usuarioId, entidade, de, ate]);
      ...
      <div className="card overflow-hidden">
        {loading ? <Spinner /> : (
          <div className="divide-y divide-slate-100">
            ...
            {lista.items.length === 0 && <p className="px-4 py-10 text-center text-slate-400">Nenhum registro.</p>}
          </div>
        )}
      </div>

      <div ref={lista.sentinelRef} />
      {lista.carregandoMais && <div className="text-center text-[12px] text-slate-400">Carregando mais...</div>}
    </div>
  );
}

...
export default function UsuarioForm() {
  ...
  async function testarSmtp() {
    const para = window.prompt('Enviar e-mail de teste para qual endereco?');
    if (!para) return;
    setTestandoSmtp(true);
    try {
      const r = await api.post<{ enviado: boolean; remetente: string }>(`/usuarios/${id}/smtp/testar`, { para });
      toast(r.enviado ? 'ok' : 'erro', r.enviado ? `Teste enviado de ${r.remetente}. Confira a caixa de ${para}.` : 'Nao enviou (SMTP nao configurado).');
    } catch (e) { toast('erro', e instanceof ApiError ? e.message : 'Falha no teste (verifique host/porta/usuario/senha e salve antes).'); }
    finally { setTestandoSmtp(false); }
  }
        ...
        {showEmail && (
          <div className="rounded border border-slate-200 bg-caixa p-4">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <button type="button" onClick={testarSmtp} disabled={testandoSmtp} className="rounded bg-marca-500 px-3 py-1.5 text-[12px] font-medium text-white hover:bg-marca-600 disabled:opacity-50">{testandoSmtp ? 'Enviando...' : 'Enviar e-mail de teste'}</button>
              <span className="text-[11px] text-slate-400">Titan: Host <b>smtp.titan.email</b> · Porta <b>465</b> · Usuario = o e-mail · Senha = a do webmail. <b>Salve</b> antes de testar.</span>
            </div>

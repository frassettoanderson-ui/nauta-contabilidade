...
import { Users, Search, SlidersHorizontal, Printer, CalendarDays, Plus, User as UserIcon, LogIn } from 'lucide-react';
...
export default function Usuarios() {
  const { sessao, impersonar } = useAuth();
  const toast = useToast();
  const navigate = useNavigate();
  const podeUsuarios = temPermissao(sessao, 'admin_usuarios');

  async function logarComo(u: UsuarioCompleto) {
    if (!window.confirm(`Logar como ${u.nome}? Voce vera o sistema como esse usuario e podera voltar ao admin a qualquer momento.`)) return;
    try {
      await impersonar(u.id);
      toast('ok', `Agora logado como ${u.nome}.`);
      navigate('/');
    } catch (e) { toast('erro', e instanceof ApiError ? e.message : 'Erro ao logar como usuario.'); }
  }
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200 text-left text-[12px] font-semibold text-marca-600">
              ...
              {podeUsuarios && <th className="px-3 py-2 text-right text-[12px] font-semibold text-marca-600">Acoes</th>}
            </tr>
          </thead>
          <tbody>
            {lista.map((u) => (
              <tr key={u.id} className="border-b border-slate-100 odd:bg-white even:bg-slate-50 hover:bg-fundo">
                ...
                {podeUsuarios && (
                  <td className="px-3 py-2 text-right">
                    {u.id !== sessao?.usuario.id && u.ativo && (
                      <button onClick={() => logarComo(u)} title={`Logar como ${u.nome}`} className="inline-flex items-center gap-1 rounded border border-marca-300 px-2 py-1 text-[11px] font-medium text-marca-600 hover:bg-marca-50">
                        <LogIn size={13} /> Logar como
                      </button>
                    )}
                  </td>
                )}
            ...
            {lista.length === 0 && <tr><td colSpan={podeUsuarios ? 4 : 3} className="px-3 py-10 text-center text-slate-400">Nenhum usuario.</td></tr>}
          </tbody>
        </table>
      </div>

...
export default function EmpresasList() {
  ...
  // debounce simples na busca (evita refazer a cada tecla)
  const [buscaDeb, setBuscaDeb] = useState(busca);
  useEffect(() => { const t = setTimeout(() => setBuscaDeb(busca), 300); return () => clearTimeout(t); }, [busca]);

  function montarQs(p: number) {
    const qs = new URLSearchParams({ page: String(p), limit: '25', status });
  ...
  const lista = useScrollInfinito<EmpresaLista>((p) => api.get<Pagina>(`/empresas?${montarQs(p)}`), [buscaDeb, status, tagId, departamentoId, grupoId, motivoUrl, refresh]);
  ...
  return (
    <div className="-m-6 min-h-full bg-fundo p-5 text-[13px]">
      ...
      {barra === 'resp' && (
        <div className="mt-2 flex flex-wrap items-end gap-2">
          <div className="min-w-[260px] flex-1">
            <label className="mb-0.5 block text-[12px] font-medium text-slate-600">Atualizar nas {lista.total} empresas listadas o responsavel do dpto:</label>
      ...
      {barra === 'tags' && (
        <div className="mt-2 flex flex-wrap items-end gap-2">
          <div className="relative min-w-[260px] flex-1">
            <label className="mb-0.5 block text-[12px] font-medium text-slate-600">Adicionar nas {lista.total} empresas listadas as Tag's:</label>
      ...
      {!loading && !lista.hasMore && items.length > 0 && (
        <div className="mt-3 text-center text-[11px] text-slate-400">{lista.total} empresa(s) — fim da lista</div>
      )}

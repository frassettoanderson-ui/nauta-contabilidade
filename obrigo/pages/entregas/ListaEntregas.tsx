...
export default function ListaEntregas() {
  ...
  const [tick, setTick] = useState(0); // incrementa ao clicar Filtrar / recarregar
  ...
  function montarQs(p: number) {
    const statusList = Object.entries(flags).filter(([, v]) => v).flatMap(([k]) => GRUPO_STATUS[k]);
    const qs = new URLSearchParams({ page: String(p), limit: '50', ordem, dir });
    ...
    for (const k of ['prazoTecDe', 'prazoTecAte', 'prazoLegalDe', 'prazoLegalAte', 'entregaDe', 'entregaAte'] as const) if (d[k]) qs.set(k, d[k]);
    return qs.toString();
  }
  // scroll infinito: carrega mais entregas ao rolar (em vez de paginacao por botoes)
  const lista = useScrollInfinito<Entrega>((p) => api.get<Pagina>(`/entregas?${montarQs(p)}`), [tick, ordem, dir]);
  const loading = lista.loading;
  const recarregar = () => setTick((t) => t + 1);

  function filtrar() { setExpandida(null); setSel(new Set()); setTick((t) => t + 1); }
  ...
  async function dispensar(e: Entrega) {
    ...
    try { await api.post(`/entregas/${e.id}/dispensar`, { motivo }); toast('ok', 'Entrega dispensada.'); recarregar(); }
  ...
  async function justificarAtraso(e: Entrega) {
    ...
    try { await api.post(`/entregas/${e.id}/justificar-atraso`, { motivo: motivo.trim() }); toast('ok', 'Atraso justificado.'); recarregar(); }
    catch (err) { toast('erro', err instanceof ApiError ? err.message : 'Erro'); }
  }
  async function entregaRapida(e: Entrega) {
    try {
      ...
      toast('ok', 'Entrega rapida registrada.'); recarregar();
    } catch (err) { toast('erro', err instanceof ApiError ? err.message : 'Erro'); }
  }
  function imprimir() {
  ...
  return (
    <div className="-m-6 min-h-full bg-fundo p-4 text-[13px]">
          ...
          <thead>
            <tr className="border-b border-slate-200 bg-white text-left align-top text-[12px] text-slate-500">
              ...
              <th className="px-3 py-2 text-right font-normal align-middle">
                <span className="text-marca-600">&laquo; {lista.total} reg &raquo;</span>
              </th>
            </tr>
          </thead>
          <tbody>
                  ...
                  {aberta && (
                    <tr className="border-b border-slate-200 bg-slate-50">
                      <td colSpan={ncols} className="px-3 py-3">
                        <LinhaBaixa entrega={e} nomeUsuario={nomeUsuario} onBaixado={() => { setExpandida(null); recarregar(); }} onDispensar={() => dispensar(e)} />
                      </td>
                    </tr>
                  )}
                </Fragment>
      ...
      {/* scroll infinito: sentinela + indicador de carregamento */}
      <div ref={lista.sentinelRef} />
      {lista.carregandoMais && <div className="mt-3 text-center text-[12px] text-slate-400">Carregando mais...</div>}
      {!loading && !lista.hasMore && items.length > 0 && (
        <div className="mt-3 text-center text-[11px] text-slate-400">{lista.total} registro(s) — fim da lista</div>
      )}

...
export default function Catalogo() {
  ...
  // obrigacoes que o robo sabe identificar (tem assinatura) -> {id da config, se aprendida via IA}
  const ecMap = new Map<string, { id: string; ia: boolean }>();
  for (const a of assinaturas) {
    const info = { id: a.id, ia: a.origem === 'IA' };
    if (a.obrigacaoNome) ecMap.set(a.obrigacaoNome, info);
    for (const oc of a.obrigacoesCorrespondentes ?? []) ecMap.set(oc, info);
  }
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {lista.map((o) => {
                ...
                const ec = ecMap.get(o.nome) ?? ecMap.get(o.id);
                return (
                  <tr
                    key={o.id}
                  ...
                  >
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-1.5 font-medium text-marca-700">
                        ...
                        {ec && (
                          <button onClick={(e) => { e.stopPropagation(); navigate(`/robo/assinaturas/${ec.id}`); }}
                            title={ec.ia ? 'O robo aprendeu a identificar este documento (via IA)' : 'Documento mapeado no robo'}
                            className={`inline-flex shrink-0 items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${ec.ia ? 'bg-emerald-100 text-emerald-700' : 'bg-marca-100 text-marca-700'}`}>
                            <Bot size={11} />{ec.ia ? 'IA' : ''}
                          </button>
                        )}
                      </div>

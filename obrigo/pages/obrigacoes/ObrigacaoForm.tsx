...
export default function ObrigacaoForm() {
  ...
  // contexto vindo da Revisao do robo (botao "Criar obrigacao"): nome pre-preenchido + abrir o
  // painel Avulsa ja na competencia certa para gerar a demanda apos salvar.
  const nav = (location.state ?? {}) as { prefillNome?: string; avulsaYm?: string; openAvulsaYm?: string; voltarPara?: string };
  ...
  // pre-preenchimento / abertura do Avulsa vindos da Revisao do robo
  useEffect(() => {
    if (novo && nav.prefillNome) setNome(nav.prefillNome);
    if (!novo && nav.openAvulsaYm) {
      setAvulsaInicio(nav.openAvulsaYm);
      setAvulsaFim(nav.openAvulsaYm);
      setAvulsaAberto(true);
    }
  ...
  async function salvar(depois: 'voltar' | 'nova') {
    ...
    try {
      ...
      if (novo) {
        const criada = await api.post<{ id: string }>('/obrigacoes', payload);
        toast('ok', 'Obrigacao salva.');
        // veio da Revisao do robo: vai para a obrigacao criada com o Avulsa ja aberto p/ gerar a demanda
        if (nav.avulsaYm) {
          navigate(`/obrigacoes/${criada.id}`, { state: { openAvulsaYm: nav.avulsaYm, voltarPara: nav.voltarPara } });
          return;
        }
        if (depois === 'nova') { navigate(0); return; }
        navigate('/obrigacoes');
        return;
      }
      await api.put(`/obrigacoes/${id}`, payload);
      toast('ok', 'Obrigacao salva.');
  ...
  async function gerarAvulsa() {
    ...
    try {
      ...
      // se veio da Revisao do robo, volta pra la (a demanda ja existe; e' so dar baixa)
      if (nav.voltarPara) navigate(nav.voltarPara);
    } catch (e) { toast('erro', e instanceof ApiError ? e.message : 'Erro ao gerar.'); }

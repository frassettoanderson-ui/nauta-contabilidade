...
// Apelido e-Continuo da empresa (igual ao que o robo usa p/ organizar as pastas).
function apelidoEmpresa(e: EmpresaLista): string {
  return e.apelidoEcontinuo || e.nomeFantasia || e.razaoSocial;
}
...
export default function Revisao() {
  ...
  useEffect(() => {
    carregar();
    api.get<{ items: EmpresaLista[] }>('/empresas?limit=300').then((p) => setEmpresas(p.items)).catch(() => undefined);
    api.get<Obrigacao[]>('/obrigacoes').then(setObrigacoes).catch(() => undefined);
    // atualiza a lista quando o botao flutuante global "Processar Drive" rodar
    const aoProcessar = () => carregar();
    window.addEventListener('drive:processado', aoProcessar);
    return () => window.removeEventListener('drive:processado', aoProcessar);
  }, []);
  ...
  // empresas ordenadas pelo apelido (mais util na busca)
  const empresasOrdenadas = [...empresas].sort((a, b) => apelidoEmpresa(a).localeCompare(apelidoEmpresa(b)));
...
function ItemRevisao({ job, empresas, obrigacoes, onPrevia, onResolvido, onReprocessado, onExcluido }: {
  job: RoboJob; empresas: EmpresaLista[]; obrigacoes: Obrigacao[];
  onPrevia: () => void; onResolvido: () => void;
  ...
  // Criar obrigacao = leva para a tela oficial de cadastro de obrigacao (a mesma do menu Obrigacoes),
  // ja com o nome preenchido e o contexto (empresa + competencia) para gerar a demanda pelo painel "Avulsa".
  function irCriarObrigacao() {
    const ym = `${ano}-${String(mes).padStart(2, '0')}`;
    navigate('/obrigacoes/nova', {
      state: {
        prefillNome: obrigacaoNome || '',
        avulsaYm: ym, // abre o painel Avulsa nessa competencia apos salvar (empresa se escolhe la)
        voltarPara: '/robo/revisao',
      },
    });
  }
    ...
    <div className="card p-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button className="btn-ghost border border-slate-300" onClick={irCriarObrigacao} title="Abre a tela de cadastro de obrigacao (a mesma do menu Obrigacoes), ja com o nome preenchido. La voce configura os prazos e usa o painel Avulsa para gerar a demanda.">Criar obrigacao</button>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-4">
        <label className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-slate-500">Empresa</span>
          <select className="input" value={empresaId} onChange={(e) => setEmpresaId(e.target.value)}>
            <option value="">Selecione...</option>
            {empresas.map((e) => <option key={e.id} value={e.id}>{apelidoEmpresa(e)}</option>)}
          </select>
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-slate-500">Obrigacao</span>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-slate-500">Competencia (mes)</span>
          <select className="input" value={mes} onChange={(e) => setMes(Number(e.target.value))}>
            {MESES.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
          </select>
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-[11px] font-medium text-slate-500">Competencia (ano)</span>
          <input type="number" className="input" value={ano} min={2000} max={2100} onChange={(e) => setAno(Number(e.target.value))} />
        </label>
      </div>
      ...
      <div className="mt-2 flex items-center justify-end gap-2">
        <span className="mr-auto text-[11px] text-slate-400">Dica: <b>Mapear</b> ensina o robo (vale pro futuro). <b>Criar obrigacao</b> abre o cadastro de obrigacao. <b>Dar baixa</b> resolve so este documento.</span>
